...
public class BlueMiddle {
    ...
    Pose2d purplePosition=new Pose2d( 37.8 , -13 ,-Math.PI/2);
    Pose2d coridorPosition=new Pose2d( 51.2 ,-15 , -Math.PI/2);
    Pose2d beforeYellowPosition=new Pose2d(51.2 ,75 , -Math.PI/2);
    Pose2d yellowPosition=new Pose2d(41.25 ,91.8 , -2.27);
    ...
    public void run(HardwareMap hardwareMap, Telemetry telemetry) {
        ...
        if(!init)
        {
            ...
            beforeGoToStack.addConditions(
                    ()->{
                        if(driveTrain.localizer.getPoseEstimate().getY()<87)
                        outtake.goDOWN();
